...
function buildBar({ user, branding, hasOwnPicture }) {
  ...
  const sidebarLinks = items.map((it) => {
    ...
    return `<a class="appshell__nav-link${active}" href="${it.href}">${escapeHtml(it.label)}</a>`;
  }).join('');
  ...
  const drawerLinks = items.map((it) => {
    ...
    return `<a class="appshell__drawer-link${active}" href="${it.href}">${escapeHtml(it.label)}</a>`;
  }).join('');
  ...
  header.innerHTML = `
    <div class="appshell__header-inner">
      ...
      <h1 class="appshell__title">
        ...
        <span class="appshell__title-app">Time management</span>
      </h1>
      ...
      <div class="appshell__menu" role="menu" hidden>
        ...
        <a class="appshell__menu-item" role="menuitem" href="/employees/${user.id}">View my profile</a>
        <a class="appshell__menu-item" role="menuitem" href="/preferences">Preferences</a>
        <button class="appshell__menu-item appshell__menu-item--danger" role="menuitem" type="button" data-action="logout">Sign out</button>
      </div>
    </div>
  `;
